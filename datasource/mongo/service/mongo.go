...
func (ds *DataSource) initialize() error {
	...
	// create model index and validator
	EnsureDB()
	// init cache
	ds.initStore()
	return nil
...
func EnsureService() {
	...
	serviceIDIndex := mutil.BuildIndexDoc(
		mutil.StringBuilder([]string{model.ColumnService, model.ColumnServiceID}))
	serviceIDIndex.Options = options.Index().SetUnique(true)

	serviceIndex := mutil.BuildIndexDoc(
		mutil.StringBuilder([]string{model.ColumnService, model.ColumnAppID}),
		mutil.StringBuilder([]string{model.ColumnService, model.ColumnServiceName}),
		mutil.StringBuilder([]string{model.ColumnService, model.ColumnEnv}),
		mutil.StringBuilder([]string{model.ColumnService, model.ColumnVersion}),
		model.ColumnDomain,
		model.ColumnProject)
...
func EnsureInstance() {
	...
	instanceServiceIndex := mutil.BuildIndexDoc(mutil.StringBuilder([]string{model.ColumnInstance, model.ColumnServiceID}))
