...
func (ds *DataSource) SearchProviderDependency(ctx context.Context, request *discovery.GetDependenciesRequest) (*discovery.GetProDependenciesResponse, error) {
	...
	if err != nil {
		if errors.Is(err, datasource.ErrNoData) {
			log.Debug(fmt.Sprintf("query provider service failed, there is no provider %s in model", providerServiceID))
		...
		log.Error(fmt.Sprintf("query provider from model error, provider is %s", providerServiceID), err)
		return nil, err
	}
	if provider == nil {
...
func (ds *DataSource) SearchConsumerDependency(ctx context.Context, request *discovery.GetDependenciesRequest) (*discovery.GetConDependenciesResponse, error) {
	...
	if err != nil {
		if errors.Is(err, datasource.ErrNoData) {
			log.Debug(fmt.Sprintf("query consumer service failed, there is no consumer %s in model", consumerID))
		...
		log.Error(fmt.Sprintf("query consumer from model error, consumer is %s", consumerID), err)
		return nil, err
	}
	if consumer == nil {
